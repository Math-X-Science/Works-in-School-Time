...
def translate_txt(file, output, max_len, batch_size, translate_batch, is_terminated):
    ...
    try:
        ...
        print(
            f"Error decoding file: {file}. Please ensure that the file is encoded in UTF-8."
        )
...
def translate_epub(file, output, max_len, batch_size, translate_batch, is_terminated):
        ...
        for text, (_, matches, pre_end) in zip(texts, text_batch):
            if text is not None:
                ...
                if len(text) < len(matches):
                    ...
                    text = text[: len(matches) - 1] + [
                        "<br/>".join(text[len(matches) - 1 :])
                    ]
    ...
    for file in files:
        ...
        try:
            ...
            with open(file, "r", encoding="utf-8") as f:
                file_text = f.read()
                matches = re.finditer(
                    r"<(h[1-6]|p|a|title).*?>(.+?)</\1>", file_text, flags=re.DOTALL
                )
                if not matches:
                    continue
                new_file_text = ""
    ...
    if not is_terminated():
        with zipfile.ZipFile(output, "w", zipfile.ZIP_DEFLATED) as f:
            for file_path in glob.glob("./temp/**", recursive=True):
...
class Translator:
    def __init__(self, model_dir, device="cpu"):
        ...
        self.model.load_state_dict(
            torch.load(f"{model_dir}/model.pth", map_location=device)
        )
        self.model.eval()
        ...
        self.encode, _ = self.src_tokenizer(
            f'{model_dir}/{self.config["vocab_path"][0]}'
        )
        _, self.decode = self.tgt_tokenizer(
            f'{model_dir}/{self.config["vocab_path"][1]}'
        )
    ...
    def translate(
        self, text, beam_size=3, device="cpu", input_cleaner=None, output_cleaner=None
    ):
        text = self.translate_batch(
            [text], beam_size, device, input_cleaner, output_cleaner
        )
        if text:
            return text[0]
        return None
    ...
    def translate_batch(
        ...
        if input_cleaner:
            text = [
                getattr(cleaner, input_cleaner)(text_single) for text_single in text
            ]

        src_tokens = rnn_utils.pad_sequence(
            (torch.LongTensor([bos_idx] + self.encode(t) + [eos_idx]) for t in text),
            batch_first=True,
            padding_value=pad_idx,
        ).to(device)
        src_mask = (src_tokens != pad_idx).unsqueeze(-2).to(device)

        results, _ = beam_search(
...
class SakuraTranslator:
    ...
    def translate_txt(
        self, file, output, input_cleaner=None, output_cleaner=None, **kwargs
    ):
